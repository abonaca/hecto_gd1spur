...
\caption{\small In an idealized scenario, a stellar stream interacting with a massive perturber is extended in the direction of the closest encounter, $(x,y)=(0,-100\,\rm pc)$. 
...
Although the exact configuration depends on the viewing angle, {\bf loops of stream stars formed following a perturbation are in general both spatially and kinematically offset from the main stream} (\emph{right}). 
Predicted differences in radial velocity of a few km\,s$^{-1}$ are easily resolved using the Hectochelle spectrograph.
}
\label{fig:loops}
\end{center}
\end{figure*}
